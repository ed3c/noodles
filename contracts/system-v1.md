...
- `edit-body` is the only admitted writer of an Issue body outside the lifecycle transitions: every provider-body edit in a multi-writer system is a compare-and-swap, so the carrier owns the guard and the caller owns only the intent and the expected-before digest. It edits only when the live body's sha256 equals a full 64-hex `--expected-before`; on drift it refuses, persists the live bytes to the caller-named `--refused-body` path for diffing, and leaves the body untouched. Resolution is the caller's judgment: there is no retry and no merge. The after-digest is asserted against a fresh provider read and both digests leave as full 64-hex, so a truncated receipt cannot witness what it guarded.

Admission boundary: the `ceremony_*` entrypoints in `noodles.py` with planted controls in `tests/test_supervised_ceremony.py` — a planted foreign git config that must not reach the commit, a rejected commit whose staged paths must be gone afterwards, a conflicting rebase that must abort and leave no in-progress state, a stale-head run id that must be refused, a carrier-routed `gh` invocation, a plant/unplant round trip over a file that also carries unrelated uncommitted edits whose bytes are compared directly before and after in both directions, a reversal whose hunk no longer applies that must fail closed leaving the file byte-identical, the ritual step in `.agents/skills/execute/SKILL.md` naming the carrier, and for `edit-body` a drifted live body that must be refused-and-persisted with the target unchanged, an elided expected-before digest that must fail before any provider call, and a provider storing other bytes that must fail the after-readback.
