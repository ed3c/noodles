...
Trusted-transition rejection - a candidate changing a value the default-branch verifier pins - has exactly one supported path, the staged transition: widen acceptance on the default branch, then flip the pinned value, then retire the widened acceptance. Rerunning, rebasing, or re-pushing the same candidate cannot resolve it, because the rejecting verifier is the default branch's, not the candidate's. The one instance this repository physically paid for is the receipt this rule keeps: pull request `ed3c/noodles#129` de-branded `.agents/skills/execute/SKILL.md` and fixed its own copy of the control that pinned the brand, and still failed at `.trusted/tests/test_noodles.py:298` in verify run `33366751879`, job `Run trusted positive and planted-negative controls`, because `.github/workflows/verify.yml` runs the default branch's test modules (`PYTHONPATH: .trusted`) over the candidate tree (`NOODLES_CANDIDATE_ROOT: .candidate`). That receipt is history, never current provider state: it records why the rule exists, and the two atoms that then paid the staged transition in order - `ed3c/noodles#252` widened the control, `ed3c/noodles#253` flipped the pinned value and retired the widening - are named for provenance, not as a status claim about either record.
