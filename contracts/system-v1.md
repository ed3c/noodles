...
- `edit-body` is the admitted Issue-body writer outside lifecycle transitions: it requires a full expected-before SHA-256, refuses and persists live bytes on drift, performs no retry, and reports success only after the provider body reads back at the intended digest.
