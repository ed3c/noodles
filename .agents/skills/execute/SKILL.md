...
14. Run `./noodles issue handoff owner/repo#N --pr N`. This validates the exact PR head/body, admits worktree provenance, sets the Issue to `awaiting_land`, and emits one blocking `stage_message` for the current `NOODLE_SESSION_ID`. It is this route's ceremony, not a gate other routes can skip: the declared feature's journey map is compiled by `./noodles github verify-pr` for every candidate, whichever writer set `awaiting_land`.
