...
column in the `gen_ledger` entry schema at the next batch append.
...
Snapshot taken 2026-09-03; 39 open issues in `ed3c/noodles` (38 at the moment
this atom's own issue was filed) plus 5 in `ed3c/ai-content-notes`. This is a
**snapshot, not a live view.** Reconstruct at any time with:
...
### 6.1 Operator-owned five — the landing surface

All five carry `noodles-state: blocked` with a `noodles-blocker: operator: ...`
naming the same owner: **the landing surface is owned by the operator's
parallel-topology line, which is its single writer.** They are filed for
visibility with receipts; they are the operator's to adopt, rescope, or
supersede. A wave lane must never implement one, and must never touch an
`issue-NNN-*` branch or PR.
...
| `#394` | `DELIVERY.LANDING_TRAIN.001` | the strict treadmill taxes a depth-`n` queue `n²` verify runs: wire verified-batch into the train's clear-queue path |
...
2. the wave's judge verdict JSON (`unmerged_handoff_verdict`);
3. the lane reports.
