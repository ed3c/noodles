...
`ed3c/noodles#390`: "a path outside the declared boundary" is decided against `policy/co-mandates.json`, not against the marker alone. Two standing mandates each force bytes outside every declared boundary — the trusted-literal guard demands a same-commit row in `policy/trusted-literals.json` from any commit that touches the trusted suite, and the cross-surface edge disclosure lives in a sentence of this document that no atom's boundary names — so the boundary contract and the two mandates were individually correct and jointly unsatisfiable. Four atoms were stranded on it and eleven boundaries were widened by hand as the interim cure, which is the per-author memory this system eliminates on principle. Each registry row names one co-permitted surface, the mandate that forces it, the change-set globs that make that mandate apply, a written reason and the live receipt that paid for the row; `issue_contract.boundary_escapes` is the ONE place that reads it, so a lane filter, `gha_apply_admission`, and any future gate inherit the same answer rather than each restating the rule. The exemption stays exactly as narrow as the mandate: the same write without its forcing condition still escapes, an unregistered path always escapes, a row whose `forced_by` is the whole tree is refused by `./noodles verify` rather than obeyed, and a row the schema gate would red co-permits nothing in the meantime, because one rule serves the gate and the judge. Retire a mandate, delete its row, and the exemption dies with it — a row naming a surface the tree no longer carries reds in `tests/test_issue_contract.py::CoMandateRegistryTests`. The rows the judge reads come from the trusted checkout and never from the candidate, because a candidate supplying the list of surfaces it may write is self-authorization; a candidate needing a NEW row lands it as its own atom and widens its declared marker meanwhile, so this is a staged transition rather than the trusted-side deadlock `ed3c/noodles#285` named. Stated ceiling, because one row over-approximates: the forcing condition is read off the changed-path set, which is all a judge standing in front of a commit has, so the AGENTS.md row is tied to the measurement's INPUT — `*.py`, exactly the set `standing_cross_surface_edges` walks — rather than to the measurement. A Python change that moved no disclosed count is co-permitted here although the disclosure did not in fact demand it; a change touching no Python module at all provably could not have moved a count and is co-permitted nothing. Tightening that needs the counts themselves, which a judge holding only a changed-path list cannot compute.
