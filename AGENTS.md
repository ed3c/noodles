...
`./noodles preflight` is the admission boundary those entrypoints run behind, and its authentication probe is bound to the ACCESS rather than to `gh`. `ed3c/noodles#434` measured the difference live: an environment that could git-fetch and reach GitHub through a connector, but carried no `gh` binary, was refused by a gate whose question it already answered. `CREDENTIAL_ADAPTERS` is the roster preflight walks — an adapter returns `None` when its transport is absent, so the next one is asked, and raises when its transport is present but the ambient identity does not resolve the target. No present transport at all still refuses, naming every transport tried, so the gate fails closed exactly where it did before. The App-token adapter is declared headroom: it lands as its own atom by appending one row.

Widening a gate is where a bypass gets in, so the no-substitution half is mechanical rather than reviewed: `credential_adapter_errors` reads the candidate's own `noodles.py` and reds any `_credential_adapter_*` that takes a credential parameter, stores into a credential-named environment key, passes a credential-named keyword to its transport, or names a credential-issuing endpoint. Ceiling, stated rather than implied: it reads those four written positions inside the adapter's own body, so an adapter that obtained a credential elsewhere and forwarded it in an ordinary header mapping is invisible to it — the refusal is of the written shape of substitution, never of the author's motive.
