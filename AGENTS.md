...
Non-claim: a definition with no entry is unowned and this check stays silent on it, so the map's coverage is exactly its honesty. `tests/test_component_surface.py::ComponentOwnerMapTests` holds the entries that do exist to real components, real definitions, and files more than one component admits; it does not require the map to be complete. A file the candidate no longer carries at all is a file-level event judged by `policy/fitness.json` `required_paths`, not a definition-level crossing; deleting a definition inside a kept file is a crossing. `noodles.py` currently carries 139 top-level definitions and the map names 18 (11 `schedule`, 7 `verify`); the other 121 are unowned and nothing reds when that count grows, so the map can only ever get more honest by someone choosing to add an entry, never by anything mechanical noticing drift (the recomputing test below already caught this exact number moving twice: 132→136 from unrelated landing-train traffic on `main`, then 136→139 from `ed3c/noodles#120`'s own `declared_markers`/`requirement_definition_source`/`system_requirement_ids`, while reconciles were in flight both times). A ratchet on that count, and closing the same-shaped `contract` (`glob "*"`) declarability gap in the paragraph above, are both filed at `ed3c/noodles#278`.
