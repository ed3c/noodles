...
The lander is the one shared singleton no worktree isolates, so `ed3c/noodles#433` pins the logic it
runs instead of pretending the entry can be isolated. `workflow_run` always executes the DEFAULT
BRANCH's copy of `.github/workflows/land.yml`; that is a platform fact, so the entry file is
unpinnable by construction and its residue is enumerated rather than argued away. Its landing job is
now a shim: one step, `python3 noodles.py github lander-checkout`, resolves `policy/github.json`'s
`lander_pin` out of the trusted default-branch checkout it was handed, checks that exact revision out
over itself, reads `HEAD` back against the pin, and exports it as `NOODLES_LANDER_PIN`. Every later
step in that job — the merge, the Issue closure, the landing train — therefore runs the pinned tree,
and `land_pull_request` reads the pin back against its own `HEAD` and records `lander` in the land
receipt, refusing before the merge when the two disagree or when the handoff never ran. The invariant
holds for the pin exactly as it holds for every other trusted value: the entry reads it from `main`,
so a candidate that moves the pin is landed by the pin `main` currently carries. Two consequences
worth naming, because they change how lander atoms are scheduled: a lander change now MERGES dead —
the pin is what decides which bytes land, so merging one alters nothing until the pin moves — and
rollback is that same one value moving back, never a revert commit passing through the queue the
defect may have broken. The drain-gate discipline moves with it: it applies to ACTIVATION moments,
not to merges. Non-claims. This changes WHERE landing logic is read from and never what it does; the
five-action bundle, protection, and App identity are untouched. Exactly one pin is live at a time —
there is no multi-version routing. The first pin points at #433's own merge base, so merging it
changed no landing behaviour, and the receipt's `lander` block starts appearing on real lands at the
first pin flip forward, not at #433's own merge. That first pin carries a one-time authoring
obligation nothing mechanical can hold, because a pin deliberately older than `main` is the steady
state and no gate can tell that case from a stale one: while #433 itself was in flight, every rebase
had to re-point the pin at the new merge base, or merging it would have silently reverted the lander
to an older revision. Landed, the obligation is gone: the pin only moves when someone flips it.
Two independent arrivals hold the
