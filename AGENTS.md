...
Non-claim: a definition with no entry is unowned and this check stays silent on it, so the map's coverage is exactly its honesty. `tests/test_component_surface.py::ComponentOwnerMapTests` holds the entries that do exist to real components, real definitions, and files more than one component admits. A file the candidate no longer carries at all is a file-level event judged by `policy/fitness.json` `required_paths`, not a definition-level crossing; deleting a definition inside a kept file is a crossing. `ed3c/noodles#326` drained that silence for the one file the map covers: `noodles.py` carries 163 top-level definitions and the map now names every one of them: 26 owned-in-place, 137 engine-root, 0 unowned. An entry's disposition class is read off its own shape rather than asserted beside it — one component is owned-in-place, and exactly the three components that admit `noodles.py` at once (`carrier`, `schedule`, `verify`) are engine-root — so `DrainedOwnershipTests` can refuse any fourth shape and a disposition cannot be minted by inventing a new list.

Non-claim on what an engine-root entry buys, because 137 rows read like 137 refusals and they are not. `component_owner_errors` runs after the file-level `component_surface_errors`, which already refuses any component whose globs do not admit `noodles.py`; `policy/components.json` admits it to exactly `carrier`, `schedule` and `verify` (plus `contract`, whose `"*"` glob short-circuits this check entirely). So a component that survives to reach an entry listing all three is by construction already in that list, and `component not in owning` cannot be true for any of the 137. An engine-root row refuses nothing on its own — it is a DISPOSITION, a written statement that this definition's evidence names no single owner, and the refusal it buys is collective: with the pool at zero, `max_unowned_top_level_definitions: 0` and `DrainedOwnershipTests`' floor mean the next definition added to this file must be dispositioned in the same commit instead of joining a silence. The 26 owned-in-place rows are the ones that refuse a crossing.

Why 137 of them are engine-root and why nothing moved to a new module: the inputs are the disclosed edges and the owner map, and a line count is not one of them. A definition is owned in place only where two independent measurements agree — its own body names component-exclusive modules of exactly one component, and every definition whose caller cone reaches it names that same one, with the argparse dispatch table excluded from the cone because routing is not coupling. The other 137 fail that on their own evidence: they name no component-exclusive module at all, or their bodies span several, or the CLI spine runs through them. Naming an owner anyway has a recorded cost and no recorded benefit — register entry 6 is a live instance where an owner narrower than the evidence blocked the very atom that would have corrected a stale comment. No disposition is `moved-with-edges-cited` because the measurement offers no move: `NoMeasuredSeamTests` walks every cross-surface module `noodles.py` imports and asks whether the definitions using it form a group nothing else reaches into and which reaches nothing else — the only shape whose extraction removes the edge instead of re-creating it as `noodles.py -> new module` for exactly the components that did not admit the target. All thirteen fail, and in only two ways: twelve have their user group held from outside, by the CLI spine or by a definition that stays behind, and the thirteenth (`schedule_domain`) has a group that reaches nineteen definitions back out, so the new module would import this one. The measurement, in the two numbers the test reads back: 13 cross-surface targets, 0 of them behind a detachable group. Every disclosed count in the paragraph above is therefore unchanged by this atom, and a split argued from size would leave them unchanged too, which is why size is not an input. That sentence is a disclosure and not a pin: `NoMeasuredSeamTests` compares it against the candidate's own tree rather than asserting the answer is empty, because asserting emptiness from the default branch would red the first honest refactor that makes a group detachable without any obligation to extract it — register entry 7's deadlock, in the file that cures it.

`ed3c/noodles#278` made the pool a number rather than a silence: `repository_metrics` reports `unowned_top_level_definitions` and `policy/fitness.json` `max_unowned_top_level_definitions` ratchets it, re-tightened by the drain to its floor of 0 so the pool cannot silently refill. The ratchet is deliberately report-only — a refusing threshold is read from the trusted default branch, so it would deadlock every candidate that legitimately adds a definition, the shape `ed3c/noodles#285` already hit. What does bite is the floor assertion in the trusted test module, which a candidate clears by dispositioning its own new definition in the same commit rather than by waiting for anything to land. That assertion could only be written after the size pin on the map itself was widened to a monotonic floor: `assertEqual(len(owned), 18)` was read from the default branch while the measurement came from the candidate, so no candidate could add an owner entry and none could merge to update the literal — registered as findings entry 7, cured as the widen half of this staged transition. #278 also closed the same-shaped `contract` (`glob "*"`) declarability gap: `contract_component_bypass_errors` refuses a whole-repository declaration whose changed files a single ordinary component already admits, keyed on the glob rather than on the name, so an atom that really spans still passes while one that declared `contract` to skip all three gates does not.
