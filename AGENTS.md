...
`ed3c/noodles#413` added the third refusal coordinate. Contract refusals live in the admission gates and architecture refusals in the ratchets above; `policy/ruff.toml` is the syntax one — a curated ruff selection wired into `verify_repository` through `lint_gate_errors` and into `tests/run.sh` ahead of the suite, with the tool version pinned by ruff's own `required-version` rather than by a dependency manifest (`policy/fitness.json` forbids every one of those). Two properties are load-bearing and neither is the other. First, the policy is read from `policy_root` while the sources are read from `root`, so a candidate that rewrites `policy/ruff.toml` is still judged by the default branch's copy and its policy change becomes authority only from the next PR onward — the one deliberate inversion of the candidate-read rule the rest of this document establishes, because a lint policy is not a value the candidate legally owns mid-verification. Second, the selection names rule groups explicitly and never `ALL`, which grows silently on every tool upgrade. Non-claim, and the honest size of today's green: existing debt is frozen at RULE granularity in that file's `ignore` list with each rule's measured count, because ruff has no per-finding baseline and the modules carrying most of those findings sit outside the declared surface of the atom that landed the gate. A green therefore means "no NEW instance of a rule that measures zero today, and no instance at all of the rules that do" — the behavioral anti-slop peers (blind-except, try-except-pass, try-except-continue, bare-except, mutable-default-argument) are all in the zero-today set. Finer-grained freezing is the type gate's baseline (`ed3c/noodles#415`, which measures per file and rule rather than per finding — see the paragraph below for why not per message), and the `S`-class selection stops at `S110`/`S112` and is never a security proof, which stays with the dataflow atom. Suppressions carry their justification in place: `PGH004` refuses a code-less directive, `RUF100` refuses one that suppresses nothing, and `lint_gate_errors` refuses one with no written reason beside it, so the only admissible shape is this repository's own comment tag followed by the rule-scoped directive.

`ed3c/noodles#415` stacked the type coordinate on it, because the production risk here is contract-shaped rather than format-shaped: `dict[str, Any]` payloads and `Mapping` crossing module seams, where "the field exists", "the type is what the caller assumed" and "`None` is impossible here" were runtime conventions no gate could read. `policy/basedpyright.json` is the trusted config and `policy/basedpyright-baseline.json` carries both the pinned tool version and the frozen debt; `type_gate_errors` wires them into `verify_repository` and `tests/run.sh` exactly where the syntax gate sits. The TRUSTED-POLICY INVARIANT is carried by a stronger mechanism than the lint gate's `--config`, because basedpyright takes its project root from the directory holding `basedpyrightconfig.json` and resolves imports from there: pointing it at the candidate would hand the candidate the config slot, and pointing it at `policy/` would break every intra-repository import (measured: 156 `reportMissingImports` and 12273 diagnostics, against 6619 once resolution works). The gate therefore materializes the candidate's tracked Python into a scratch tree and writes the trusted bytes in beside it, so the candidate is scanned as data and is never a project. Adoption is BASELINE rather than cleanup, and this is the honest size of today's green: 6652 diagnostics across 555 (file, rule) keys are frozen with their measured counts, so a green means "this tree adds no diagnostic beyond what the trusted baseline already records for that file and that rule", and every diagnostic in a file the baseline does not name reds from day one. Non-claim about the granularity: the key is (file, rule) and not (file, rule, message), because basedpyright's messages are localized — the same run emits Chinese under a `zh` locale, measured on this host — so a message-keyed baseline would be a green that depends on the runner's environment. Burn-down needs no edit here at all: the baseline is a ceiling, measuring under it is green, and an entry is deleted when its surface's owner gets to it. Growth is the direction that costs: the file is measurement-derived exactly the way `policy/definition-dispositions.json` is, so a rebase that gains diagnostics is restated from the tree rather than hand-merged, and `TypeGateTests::test_the_landed_baseline_is_what_this_tree_actually_measures` is the mechanical carrier that reds on a stale one — it asserted the freeze against the tree through the gate's own reader and caught this branch's own rebase, which moved 6670 diagnostics across 556 keys to 6747 across 562. The pin is ours rather than the tool's: basedpyright has no `required-version`, so `type_gate_errors` refuses a version mismatch itself, since an unpinned checker is a green whose meaning moves under it. Suppressions carry three independent carriers here too, and none is the other's proof: `enableTypeIgnoreComments` is false so a bare type-ignore comment suppresses nothing and the diagnostic surfaces anyway, `reportIgnoreCommentWithoutRule` refuses a rule-less directive, and `type_gate_errors` refuses the bare comment itself and admits a rule-scoped one only inside this repository's comment tag with a written reason beside it.
